...
template <class T>
void ArbolAVL<T>::inordenEnLista(std::list<T> &result)
{
    inOrdenRecursivoList(raiz, result);
}
template <class T>
...
template <class T>
void ArbolAVL<T>::buscarPrefijo(std::string prefijo, std::list<Palabra> &pPrefijo)
{
    buscarPrefijo(raiz,prefijo,pPrefijo);
}

template <class T>
void ArbolAVL<T>::buscarSubfijo(std::string prefijo, std::list<Palabra> &pPrefijo)
{
    buscarSubfijo(raiz,prefijo,pPrefijo);
}

// Métodos de inserción y eliminación
template <class T>
bool ArbolAVL<T>::insert(const T& dato)
{
    T copiaNoConst = dato;
...
template <class T>
bool ArbolAVL<T>::erase(const T& dato)
{
    T copiaNoConst = dato;
    bool status = buscar(copiaNoConst);
...
// Métodos privados para operaciones internas
    ...
    void ArbolAVL<T>::imprimirArbolGrafico(NodoAVL<T>* nodo, int nivel, const std::string& prefijo)
    {
        ...
        std::cout << std::string(espaciado * nivel, ' ') << prefijo << nodo->obtenerDato().contenido << std::endl;
    ...
    bool ArbolAVL<T>::buscarRecursivo(NodoAVL<T>* nodo, T& val) 
    {
        ...
        if (val.contenido == nodo->obtenerDato().contenido) {
            return true; // El valor se encontró en el nodo actual
        } else if (val.contenido < nodo->obtenerDato().contenido) {
    ...
    void ArbolAVL<T>::preOrdenRecursivo(NodoAVL<T>* nodo) 
    {
        if (nodo != nullptr) {
            // Visitar el nodo actual
            std::cout << nodo->obtenerDato().contenido << " ";
    ...
    void ArbolAVL<T>::inOrdenRecursivo(NodoAVL<T>* nodo) 
    {
        if (nodo != nullptr) {
            ...
            // Visitar el nodo actual
            std::cout << nodo->obtenerDato().contenido << endl;
            // Recorrer el subárbol derecho
    ...
    template <class T>
    void ArbolAVL<T>::inOrdenRecursivoList(NodoAVL<T>* nodo, std::list<T> &result) 
    {
        if (nodo != nullptr) {
            // Recorrer el subárbol izquierdo
            inOrdenRecursivoList(nodo->obtenerHijoIzq(), result);
            // Visitar el nodo actual
            result.push_back(nodo->obtenerDato());
            // Recorrer el subárbol derecho
            inOrdenRecursivoList(nodo->obtenerHijoDer(), result);
        }
    }
    ...
    template <class T>
    void ArbolAVL<T>::buscarPrefijo(NodoAVL<T> *nodo,std::string prefijo, std::list<Palabra> &pPrefijo)
    {
        if(nodo == nullptr)
        {
            return;
        }
        if(nodo->obtenerDato().contenido.compare(0, prefijo.size(), prefijo) == 0)
        {
            pPrefijo.push_back(nodo->obtenerDato());
            buscarPrefijo(nodo->obtenerHijoIzq(), prefijo, pPrefijo);
            buscarPrefijo(nodo->obtenerHijoDer(), prefijo, pPrefijo);
        }
        else if(nodo->obtenerDato().contenido < prefijo)
        {
            buscarPrefijo(nodo->obtenerHijoDer(), prefijo, pPrefijo);
        }
        else
    ...
    template <class T>
    void ArbolAVL<T>::buscarSubfijo(NodoAVL<T> *nodo,std::string subfijo, std::list<Palabra> &pSubfijo)
    {
        if(nodo == nullptr)
        {
            return;
        }
        if(nodo->obtenerDato().contenido.length() >= subfijo.length() && nodo->obtenerDato().contenido.substr(nodo->obtenerDato().contenido.length()
           - subfijo.length()) == subfijo)
        {
            pSubfijo.push_back(nodo->obtenerDato());
        }
        buscarSubfijo(nodo->obtenerHijoIzq(), subfijo, pSubfijo);
        buscarSubfijo(nodo->obtenerHijoDer(), subfijo, pSubfijo);
    ...
    NodoAVL<T>* ArbolAVL<T>::insertarRecursivo(NodoAVL<T>* nodo, const T& dato)
    {
        ...
        if (dato.contenido < nodo->obtenerDato().contenido) {
            nodo->fijarHijoIzq(insertarRecursivo(nodo->obtenerHijoIzq(), dato));
        } else if (dato.contenido > nodo->obtenerDato().contenido) {
